...
static bool compare_sleep_ticks(struct list_elem *a, struct list_elem *b);   //prototype function for the comparator

struct list t_list;    //List to keep track of threads
...
bool
compare_sleep_ticks(struct list_elem *a, struct list_elem *b){
  struct thread *t_a;
  struct thread *t_b;
...
timer_sleep (int64_t ticks) 
{
  ...
  t->sleep_ticks = end;   //assign sleep time to current thread
  
  intr_disable();   
  list_insert_ordered(&t_list, &t->elem, compare_sleep_ticks, NULL);
  thread_block();
  intr_enable();
}
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  while(!list_empty(&t_list)) {
    t = list_entry(list_back(&t_list),struct thread, elem);
