...
#define Q_FP (1 << FIX_Q)      /* pow(2, FIX_Q). */
...
bool sort_priority_list_less(const struct list_elem *a,const struct list_elem *b, void *aux )
{
  int64_t p_a = list_entry(a, struct thread, elem)->priority;
  int64_t p_b = list_entry(b, struct thread, elem)->priority;
  return p_a > p_b;
}


bool sort_priority_list_after(const struct list_elem *a,const struct list_elem *b, void *aux )
{
  int64_t p_a = list_entry(a, struct thread, elem)->priority;
  int64_t p_b = list_entry(b, struct thread, elem)->priority;
  return p_a >= p_b;
...
{
  list_sort(&ready_list,sort_priority_list_less,NULL);
}
...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_ordered(&ready_list, &t->elem, sort_priority_list_less, NULL);
...
thread_yield (void) 
{
  ...
  {
    if(thread_mlfqs)
      list_insert_ordered(&ready_list, &cur->elem, sort_priority_list_after,NULL );
    else
...
{
  load_avg = multiply_fp(convert_to_fp(59)/60, load_avg) + (convert_to_fp(1)/60)*(thread_current() == idle_thread ? list_size(&ready_list): list_size(&ready_list)+1);
